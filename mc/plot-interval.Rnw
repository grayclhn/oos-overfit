...
<<testerror1, fig=TRUE, width=6, eps=F, pdf=T>>=
...
% naive out-of-sample t-test-based interval.
% <<testerror2, fig=TRUE, width=6, tikz=T, eps=F, pdf=T>>=
...
naive out-of-sample t-test-based interval.
<<generror1, fig=TRUE, width=6, eps=F, pdf=T>>=
...
% naive out-of-sample t-test-based interval.  <<generror2, fig=TRUE,
% width=6, tikz=T, eps=F, pdf=T>>= d <- dbGetQuery(dbc,
% sprintf(dstring, 'gen.error.2', 500)) d$nlabel <-
% sprintf("T=%d", d$n)
% d$normlabel <- sprintf("c=%d", d$norm)
