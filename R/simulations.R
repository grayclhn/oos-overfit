...
varfn[["rmse.difference"]] <- function(L, vcv) calcvar(vcv, c(0.5/sqrt(L[1]), -0.5/sqrt(L[2])))

locfn[["rmse.ratio"]] <-      function(L) sqrt(L[1] / L[2])
varfn[["rmse.ratio"]] <-      function(L, vcv) calcvar(vcv, c(0.5/sqrt(L[1]*L[2]), -0.5*sqrt(L[1]/(L[2]^3))))

locfn[["log.difference"]] <-  function(L) log(L[1]) - log(L[2])
varfn[["log.difference"]] <-  function(L, vcv) calcvar(vcv, c(1/L[1], -1/L[2]))

getmoments <- function(x, lfn, vfn) {
...
cw.moments <- function(x,...) {
  mapply(function(e1, e2, f1, f2) {
    ...
    c(loc = avg.loss, var = var.loss)
         ...
         e2 = ForecastErrors(model.alt(x)),
         f1 = forecasts(model.null(x)),
         f2 = forecasts(model.alt(x)))
}

tstat <- function(n, moments, mu)
  (moments[1,] - mu) * sqrt((n-1)/moments[2,])

ztest <- function(n, moments, mu, conf.level, one.sided=rep(TRUE, length(conf.level))) {
...
add.teststat <- initTable(dbc, intervaltable, function(statlist, isim, idgp, simIndex, ntest, label) {
  if (length(label) == length(statlist)) {
    do.call(rbind, lapply(seq_along(statlist), function(x) {
      data.frame(ntest = as.integer(ntest), isim = as.integer(isim), idgp = as.integer(idgp), simIndex = as.integer(simIndex),
                 reject = statlist[[x]], transform = names(statlist[x]), label = unname(label[x]))}))
  } else {
    do.call(rbind, lapply(seq_along(statlist), function(x) {
      data.frame(ntest = as.integer(ntest), isim = as.integer(isim), idgp = as.integer(idgp), simIndex = as.integer(simIndex),
                 reject = statlist[[x]], transform = names(statlist[x]), label = unname(label))}))
  }})

add.normalizeddiff <- initTable(dbc, difftable, function(statlist, isim, idgp, simIndex, ntest) {
  do.call(rbind, lapply(seq_along(statlist), function(x) {
    data.frame(ntest = as.integer(ntest), isim = as.integer(isim), idgp = as.integer(idgp), simIndex = as.integer(simIndex),
               value = statlist[[x]], transform = names(statlist[x]))}))})

add.ftest <- initTable(dbc, ftesttable, function(testresult, isim, idgp, simIndex, label) {
  data.frame(reject = testresult, isim = as.integer(isim), idgp = as.integer(idgp),
             simIndex = as.integer(simIndex), label)})
...
transvec <- ls(locfn)
nullerror <- sapply(transvec, function(trans) locfn[[trans]](c(1,1)), simplify = FALSE)
...
for (r in rows(mcDesign)) {
  ...
  repeat {
    ...
    add.teststat(sapply(transvec, function(trans) {
      ztest(ntest, moments[[trans]], testerror[[trans]], confidence, one.sided=TRUE)
    }, simplify = FALSE), r$isim, r$idgp, simIndex, ntest, "test.error.1")
      
    ## Add two-sided test that mu = E_R D1-bar to database
    add.teststat(sapply(transvec, function(trans) {
      ztest(ntest, moments[[trans]], testerror[[trans]], confidence, one.sided=FALSE)
    }, simplify = FALSE), r$isim, r$idgp, simIndex, ntest, "test.error.2")
      
    ## Add one-sided test that mu = E_T D2-bar to database
    add.teststat(sapply(transvec, function(trans) {
      ztest(ntest, moments[[trans]], generror[[trans]], confidence, one.sided=TRUE)
    }, simplify = FALSE), r$isim, r$idgp, simIndex, ntest, "gen.error.1")
      
    ## Add two-sided test that mu = E_T D2-bar to database
    add.teststat(sapply(transvec, function(trans) {
      ztest(ntest, moments[[trans]], generror[[trans]], confidence, one.sided=FALSE)
    }, simplify = FALSE), r$isim, r$idgp, simIndex, ntest, "gen.error.2")

    ## Add one-sided test that mu = 0 to database
    add.teststat(sapply(transvec, function(trans) {
      ztest(ntest, moments[[trans]], nullerror[[trans]], confidence, one.sided=TRUE)
    }, simplify = FALSE), r$isim, r$idgp, simIndex, ntest, simType)

    ## Add McCracken's test that mu = 0 to database
    if (r$kAlt - r$kNull <= 10) {
      add.teststat(list(mccracken = tstat(ntest, moments[["difference"]], 0) >
                        qmccracken(r$kAlt - r$kNull, ntest / (r$n - ntest))),
                   r$isim, r$idgp, simIndex, ntest, simType)
    }
    ## Add Clark and West's test that mu=0 to database
    add.teststat(list(clarkwest = ztest(ntest, cw.moments(oosDifference), 0, confidence, one.sided=TRUE)),
                 r$isim, r$idgp, simIndex, ntest, simType)
    
    ## Add difference E_T D2-bar - E_R D1-bar normalized by variance
    add.normalizeddiff(sapply(transvec, function(trans) {
      sqrt(ntest/moments[[trans]][2,]) * (generror[[trans]] - testerror[[trans]])
    }, simplify = FALSE), r$isim, r$idgp, simIndex, ntest)

    ## Add F-test to database
    add.ftest(f.test(oosDifference) < 1 - confidence, r$isim, r$idgp, simIndex, simType)
