...
\begin{abstract}\thispagestyle{empty}\noindent
  ...
  positive.  Even under this limit theory, the naive
  Diebold-Mariano-West out-of-sample test can test hypotheses about
  the models' generalization error, a key quantity for evaluating
  forecasting models.  Moreover, tests that are designed to reject if
  the larger model is true, such as the usual in-sample Wald and
  ...
  model is more accurate. Similar results hold for statistics used for
  in-sample model selection, such as the \aic, \bic, and adjusted
  $R$-square.
...
comparisons and discover a practical limitation of existing statistics.
...
conditions that permit in-sample comparisons \citep[see][for a recent
...
example,][]{Whi:00,Han:05,RoW:05,HHK:10,ClM:11}, but it is unclear
...
asymptotic theory, the \ols\ coefficient estimator is no longer
consistent or asymptotically normal \citep{Hub:73} and has positive
variance in the limit.  Even so, we show that the usual \oos\ average
...
benchmark is more accurate.\footnote{In a pair of papers similar to
  ours, \citet{ClM:09b,ClM:12} study in-sample and \oos\ tests that
  ...
  asymptotic theories are different and their paper does not provide a
  reason to do \oos\ comparisons.}
...
estimation, and $P^2/T \to 0$ is required for valid confidence
...
\rVert_v$.  The functions $\lambda_i(\cdot)$ take a square-matrix
argument and return its $i$th eigenvalue (with $\lambda_{i}(A) \leq
\lambda_{i+1}(A)$ for any matrix $A$).  All limits are taken as $T \to
...
\begin{asmp}\label{asmp-2}
  ...
  \|_3 \leq K_i \Delta$,
  \begin{align*}
    \lVert \lambda_{\max}(X_{iS_{T}}'X_{iS_{T}}) \rVert_{3} &\leq
    S_{T}\, \Delta, & \lVert
    \lambda_{\max}(X_{iS_{T}}'\ep{iS_{T}}\ep{iS_{T}}'X_{iS_{T}})
    \rVert_{3} &\leq S_{T}\, \Delta, \\
    \lVert \lambda_{\max}((X_{iS_{T}}'X_{iS_{T}})^{-1})
    \rVert_{3} &\leq \Delta/S_{T}, & \lVert
    \lambda_{\max}((X_{iS_{T}}'\ep{iS_{T}}\ep{iS_{T}}'X_{iS_{T}})^{-1})\rVert_{3}
    &\leq \Delta/S_{T},
  \end{align*}
...
This application to testing can be expressed as a corollary:
\begin{cor}\label{res:oostest}
Suppose that the conditions of Theorem \ref{res-confidence-intervals}
hold.  Then
\begin{equation}
  \lim \Pr[P^{1/2}\oosA/\hat\sigma_R \geq z_{\alpha} \mid \E_{T}
...
\begin{align*}
  ...
  \hat{\theta}_{2T}^T \\ &=
  \mathbf{\varepsilon}_T'[\tilde{X}_{2R}(X_{2R}'X_{2R})^{-2}
  \tilde{X}_{2R} - X_{2T}(X_{2T}'X_{2T})^{-2} X_{2T} ] \mathbf{\varepsilon}_T
\end{align*}
...
\begin{thm}\label{res:ftest}
  ...
  distributions for $(x_t, \varepsilon_{t+h})$ such that
  \begin{equation}
    \label{eq:11}
...
there are distributions on $(x_t, \varepsilon_{t+h})$ that are
...
\begin{cor}\label{res:ic}
  ...
  region.  Then there exist distributions on $x_t,\varepsilon_t$ such
  that
  \begin{equation}
    \label{eq:17}
...
\begin{align*}
  ...
  (X_{iR}'X_{iR})^{-1}] X_{it}' \mathbf{\varepsilon}_{it} \big\|_2 \\
  &\quad + \big\| x_{it}^{*\prime}
  (X_{iR}'X_{iR})^{-1}[X_{it}'\mathbf{\varepsilon}_{it} - X_{iR}
  \mathbf{\varepsilon}_{iR} ] \big\|_2 \Big).
\end{align*}
  Moreover,
\begin{align*}
  \big\| x_{it}^{*\prime} [(X_{it}'X_{it})^{-1} &-
  (X_{iR}'X_{iR})^{-1}] X_{it}' \mathbf{\varepsilon}_{it} \big\|_2 \\
  &\leq \E \Big| \lambda_{K_i}(X_{it}' \mathbf{\varepsilon}_{it}
  \mathbf{\varepsilon}_{it}' X_{it})\sum_{t=R+1}^{T-h}
  \sum_{j=1}^{K_i} \lambda_j^2((X_{iR}'X_{iR})^{-1} -
  (X_{it}'X_{it})^{-1}) \lambda_{\pi_j}(x_{it}^{*} x_{it}^{*\prime})
  \Big| \\ &\leq \Delta T \Big\| \sum_{t=R+1}^{T-h} \sum_{j=1}^{t-R}
  \lambda_j^{-2}(X_{iR}'X_{iR}) \lambda_{\pi_j}(x_{it}^{*}
  x_{it}^{*\prime}) \Big\|_{3/2} \\ &\leq \Delta^3 P^2/R
\end{align*}
where $\pi_i,\dots,\pi_{K_i}$ is a permutation of $1,\dots,K_i$.  The
first equality holds because $\lambda_{\max}(X_{it}'
\mathbf{\varepsilon}_{it} \mathbf{\varepsilon}_{it} X_{it}) =
...
\begin{equation*}
  \big\| x_{it}^{*\prime}
  (X_{iR}'X_{iR})^{-1}[X_{it}'\mathbf{\varepsilon}_{it} - X_{iR}
  \mathbf{\varepsilon}_{iR} ] \big\|_2 \leq \Delta^3 P^2/R
\end{equation*}
as well, so 
\begin{equation*}
  \Big\|P^{-1/2} \sum_{t=R+1}^{T-h} L(y_{t+h}^{*} - x_{it}^{*\prime}
...
$\theta_0$.  For any distribution of $(\varepsilon_{t+h}, x_t')$ such
...
$(\varepsilon_{t+h}, x_t) \sim i.i.d. N(0,I)$ and condition on $X_T$.
