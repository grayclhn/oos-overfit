...
\author{Gray Calhoun\thanks{email: \texttt{gcalhoun@iastate.edu}. I
    ...
    with Ivo Welch \citep{goyal-welch-2008-rfs}.} \\ Iowa State
...
asymptotically normal \citep{huber_robust_1973} with positive variance
in the limit.  Even so, we show that the usual \oos\ average is
...
analogue to the generalization error.\footnote{See, for example,
  \citet{elemstatlearning}.}  Under these asymptotics, the
generalization error does not converge to the expected performance of
the pseudo-true models, so the in-sample and \oos\ comparisons measure
different quantities and should be expected to give different results
for reasons beyond simple size and power comparisons.  Under our
theory, the model that is closer to the true \dgp\ in population can
forecast worse.  In such a situation, an in-sample comparison would
...
benchmark is more accurate.\footnote{In a pair of papers similar to
  ours, \citet{RePEc:fip:fedkrw:rwp09-10,RePEc:fip:fedkrw:rwp09-11}
  study in-sample and \oos\ tests that the larger model has nonzero
...
asymptotically valid.  Moreover,
\citet{efron_biased_1986,efron_estimation_2004} shows that naive
comparisons of in-sample loss overestimate models' accuracy by a
...
shown by \citet{boos_anova_1995}, \citet{akritas_asymptotics_2000},
\citet{akritas_heteroscedastic_2004}, and
\citet{calhoun_hypothesis_2009}, among others.  Such results establish
...
\cite{clark_tests_2001,clark_evaluating_2005},
...
mixing sequences.  Please see, for example,
\citet{davidson_stochastic_1994} for a discussion of these mixing
conditions.
...
% LocalWords:  merlevede cltvar dR Scwarz davidson indices lrrrr waldtest Df
% LocalWords:  dmw McCracken MSE OLS mse
